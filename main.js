...
function callback (obj) {
    filtered = filtered.concat(getFilteredEmails(obj.result))
    if (obj.next) {
      fetchEmailsFromDatabase(obj.next, callback)
    } else {
...
function getFilteredEmails(allEmails = [], searchInputs = getSearchInputs()) {
  ...
  allEmails.forEach((email) => {
    let emailText = [email.author, email.subject, email.body].join(" ");

    searchInputs.forEach((input) => {
      if (_.includes(emailText, input)) {
